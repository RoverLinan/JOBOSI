...
public class ErrorOSIMapper {
    ...
    public static ErrorOSIBO mapToAttention(JobNameEnum jobNameEnum, AttentionRegisterRequestDTO attentionRegisterRequestDTO, AbstractResponseDTO abstractResponseDTO, Exception exception, ErrorTypeEnum errorType){
        ErrorOSIBO errorOSIBO = new ErrorOSIBO(jobNameEnum.getTableName(), attentionRegisterRequestDTO.getCodigoAtencion(),errorType.name());
    ...
    public static ErrorOSIBO mapToUploadFile(JobNameEnum jobNameEnum, Map<String, Object> entity, AbstractResponseDTO abstractResponseDTO, Exception exception, ErrorTypeEnum errorType) {
        ErrorOSIBO errorOSIBO;
        if(Objects.nonNull(entity)){
            errorOSIBO = new ErrorOSIBO(jobNameEnum.getTableName(), (String) entity.get(COD_ATENCION.getFieldName()),errorType.name());
            errorOSIBO.setCodAccion((String) entity.get(COD_ACCION.getFieldName()));
        }else{
            errorOSIBO = new ErrorOSIBO(jobNameEnum.getTableName(), "ALL",errorType.name());
